...
**Fixed:**
  - Session authenticator : `logout()` now clears the remember-me token even when the session user was never resolved during the logout request. Previously the remember cookie survived when `logout()` ran without a resolved user, so visiting the login page silently re-authenticated the user
  - Email2FA : `verify()` now checks for `admin.access` and redirects admins to `after_login_admin` after a successful code check, matching the plain login flow instead of always sending everyone to `after_login`
